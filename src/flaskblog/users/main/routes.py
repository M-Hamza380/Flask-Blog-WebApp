...
from src.flaskblog.users.forms import (
    ChangePasswordForm,
    LoginForm,
    RegistrationForm,
    RequestResetForm,
    ...
    if form.validate_on_submit():
        ...
        if user:
            if user.can_reset_password():
                ...
                flash(
                    "You have exceeded the password reset limit for today. Please try again tomorrow.",
                    category="warning",
                )
        else:
...
def reset_token():
    logger.info("Entered reset_token route.")
