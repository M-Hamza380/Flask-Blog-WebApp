...
from src.flaskblog import bcrypt, db
...
def admin_login():
    ...
    if form.validate_on_submit():
        ...
        if (
            ...
            and admin_user.email == "tecdev74@gmail.com"
            and bcrypt.check_password_hash(admin_user.password, form.admin_password.data)
        ):
            login_user(admin_user)
            next_page = request.args.get("next")
            flash("Login successful in admin panel.", category="success")
            return (
                redirect(next_page) if next_page else redirect(url_for("views.admin_home", admin_user=admin_user, form=form))
            )
        else:
